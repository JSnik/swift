...
class AuthRequest {
    ...
    static let TAG = String(describing: AuthRequest.self)
    ...
    init(_ notificationViewController: NotificationViewController, _ urlQueryItems: [URLQueryItem]) {
        ...
        let url = URL(string: Configuration.API_URL + "/auth")!
        
        GeneralUtils.log(AuthRequest.TAG, url)
        ...
        task = URLSession.shared.dataTask(with: request) { data, response, error in
            DispatchQueue.main.async(execute: {
                ...
                if let responseJson = responseJson as? [String: Any] {
                    ...
                    GeneralUtils.log(AuthRequest.TAG, String(data: data, encoding: .utf8)!)
