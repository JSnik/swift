//
//  EpisodeSubscriptionStatusPostRequest.swift
//  Latvijas Radio
//
...
class EpisodeSubscriptionStatusPostRequest {

    static let TAG = String(describing: EpisodeSubscriptionStatusPostRequest.self)
    ...
    init(_ notificationViewController: NotificationViewController?, _ episodeId: String, _ urlQueryItems: [URLQueryItem]) {
        self.notificationViewController = notificationViewController
        
        createQuery(episodeId, urlQueryItems)
        
        // network callbacks must not contain weak self
        restartRequestCallback = {
            self.createQuery(episodeId, urlQueryItems)
            self.execute()
        }
    }
    
    func createQuery(_ episodeId: String, _ urlQueryItems: [URLQueryItem]) {
        notificationViewController?.hideNotificationInstantly()
        
        let url = URL(string: Configuration.API_URL + "/episode/" + episodeId + "/subscription-status")!
        ...
        task = session.dataTask(with: request) { data, response, error in
            DispatchQueue.main.async(execute: {
                ...
                if let responseJson = responseJson as? [String: Any] {
                    ...
                    GeneralUtils.log(EpisodeSubscriptionStatusPostRequest.TAG, String(data: data, encoding: .utf8)!)
