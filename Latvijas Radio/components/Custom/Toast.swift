...
class Toast {
    ...
    static func show(message: String, controller: UIViewController) {
        ...
        toastContainer.backgroundColor = UIColor(named: ColorsHelper.BLACK_50_PERCENT) //UIColor.black.withAlphaComponent(0.75)
        toastContainer.alpha = 0.0
        toastContainer.layer.cornerRadius = 10;
        toastContainer.clipsToBounds = true
        
        let toastLabel = UILabel(frame: CGRect())
        toastLabel.textColor = UIColor(named: "lr-white")
        toastLabel.textAlignment = .center;
        //toastLabel.font = UIFont(name: "SFCompactText-Regular", size: 16)
