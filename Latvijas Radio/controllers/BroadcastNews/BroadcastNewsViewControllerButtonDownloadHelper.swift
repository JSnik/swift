//
//  BroadcastNewsViewControllerButtonDownloadHelper.swift
//  Latvijas Radio
//
...
class BroadcastNewsViewControllerButtonDownloadHelper {
    
    static var TAG = String(describing: BroadcastNewsViewControllerButtonDownloadHelper.self)

    weak var viewController: NewsCell?
    
    var asset: Asset!
    
    deinit {
        GeneralUtils.log(BroadcastNewsViewControllerButtonDownloadHelper.TAG, "deinit")
    }
    ...
    @objc func handleAssetDownloadProgress(_ notification: NSNotification) {
        ...
        GeneralUtils.log(BroadcastNewsViewControllerButtonDownloadHelper.TAG, "Download progress:", progress)
...
//        viewController!.downloadProgress.progress = Float(progress)
    ...
    func updateEpisodeDownloadButtonState() {
        ...
        if (asset.episodeModel.getMediaDownloadUrl() == nil) {
            ...
            if (usersManager.getOfflineEpisodeById(asset.episodeModel.getId()) != nil) {
                ...
                viewController!.buttonDownload.setImage(UIImage(named: ImagesHelper.IC_CHECKMARK_IN_CIRCLE_RED_ON_WHITE), for: .normal)
                ...
                if AssetPersistenceManager.sharedManager.localAssetForEpisodeModel(withEpisodeModel: asset.episodeModel) != nil {
                    ...
                    if AssetPersistenceManager.sharedManager.assetForEpisodeModel(withId: asset.episodeModel.getId()) != nil {
                        ...
                        viewController!.buttonDownload.setImage(UIImage(named: ImagesHelper.IC_DOWNLOAD_BLACK_ON_WHITE), for: .normal)
